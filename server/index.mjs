  ...
  try {
    ...
    if (!hasUsableOpenAiKey() && !demoModeEnabled) {
      return res.status(500).json({
        error: 'OPENAI_API_KEY nao configurada no arquivo .env. Ative DEMO_MODE=true para testes sem chave.',
      })
    }

    if (!hasUsableOpenAiKey() && demoModeEnabled) {
      return res.json({
        reply: `Modo demo ativo na trilha ${context.careTrack}. Recebi: "${lastUserMessage?.content || ''}". Configure OPENAI_API_KEY para respostas reais.`,
      })
    }
